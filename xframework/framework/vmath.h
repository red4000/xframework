...
public:
	...
	inline void CalculateAngles() {
		...
		if ((0 == y) && (0 == x)) {
			...
			if (x) {
				yaw = (T)RAD2DEG(atan2((double)y, (double)x));
			} else if (y > 0) {
				yaw = 90;
			} else {
				yaw = 270;
			...
			T forward = (T)sqrt((double)((x * x) + (y * y)));

			pitch = (T)RAD2DEG(atan2((double)z, (double)forward));
	...
	inline void AngleVectors(_Vector3<T> *f, _Vector3<T> *r, _Vector3<T> *u) {
		...
		angle = (T)(z * (M_PI * 2.0 / 360.0));
		sr    = sin(angle);
		cr    = cos(angle);

		angle = (T)(x * (M_PI * 2.0 / 360.0));
		sp    = sin(angle);
		cp    = cos(angle);

		angle = (T)(y * (M_PI * 2.0 / 360.0));
		sy    = sin(angle);
		cy    = cos(angle);
