...
float GetFOV(FVector3 *from, FVector3 *to, float fovX, float fovY) {
	FVector3 vec = *to;
	vec.Subtract(from);
	vec.CalculateAngles();
	vec.Subtract(&viewAngles);
	vec.Normalize180();
	if (fovX >= abs(vec.x) && fovY >= abs(vec.y)) {
		return vec.Length();
	} else {
		return 9999.f;
	}
...
bool IsInFOV(FVector3 *from, FVector3 *to, float fovX, float fovY) {
	//if (fovX == 360 && fovY == 360) {
	if ((fovX + fovY) == (360 + 360)) {
		return true;
	}
	to->Subtract(from);
	to->CalculateAngles();
	...
	to->Normalize180();
	return fovX >= abs(to->x) && fovY >= abs(to->y);
}
