...
_Color espEnemyVisCol       = D3DCOLOR_ARGB(255, 255, 25, 25),
	   espEnemyInvisCol     = D3DCOLOR_ARGB(255, 25, 255, 25),
	   espFriendlyVisCol    = D3DCOLOR_ARGB(255, 20, 200, 20),
	   espFriendlyInvisCol  = D3DCOLOR_ARGB(255, 20, 20, 255),
...
int      barrelEsp, barrelLength = 250, boxEsp, boxHairs, boxEdges, pixelEsp, pixelSize = 3, distanceEsp, explosiveEsp, itemEsp, enemiesOnly, nameEsp, weaponEsp, threatEsp, espStyle;
float    boxEdgeFrac = 0.25f;
Font    *espFont;
...
void RenderPlayerESP(int i, Entity *ent, PlayerInfo *pi) {
	...
	if (ent->IsHostile()) {
		if (pi->visible) {
			entCol = espEnemyVisCol;
		} else {
			entCol = espEnemyInvisCol;
		}
	} else {
		if (enemiesOnly) {
			return;
		...
		if (pi->visible) {
			entCol = espFriendlyVisCol;
		} else {
			entCol = espFriendlyInvisCol;
		}
	}
