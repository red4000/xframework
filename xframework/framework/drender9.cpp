...
void DX9Renderer::Render2DScissorRect(int x, int y, int w, int h, int sx, int sy, int sw, int sh, _Color c) {
	//assume(sx >= x && sy >= y && (sx + sw) < (x + w) && (sy + sh) < (y + h));
	int rw = sx - x;
	SetColor(c);
	Render2DRect(x, y, rw, h);
	Render2DRect(x + rw, y, w - rw, sy - y);
	Render2DRect(x + rw, (y + h) - ((y + h) - (sy + sh)), w - rw, sy - y);
	rw = (x + w) - (sx + sw);
	Render2DRect(x + sx + sw, y, rw, h);
}
