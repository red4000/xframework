...
void UpdateCursorPosition() {
	short nmx = *(float*)0xCAEE210;
	short nmy = *(float*)0xCAEE214;

	POINT pPoint;
	GetCursorPos(&pPoint);
	HWND GameWindow = FindWindowA(0, "Call of Duty 4");
	WINDOWPLACEMENT WinPlace;
	GetWindowPlacement(GameWindow, &WinPlace);

	nmx = pPoint.x - WinPlace.rcNormalPosition.left;
	nmy = pPoint.y - WinPlace.rcNormalPosition.top;
			...
			if (targetEnt == NULL) {
				for (size_t i = 0; i < MAX_PLAYERS; i++) {
					...
					if (ent->IsAlive()) {
						...
						pi->distance = viewOrigin.Distance(&pi->head);
						pi->visible = IsPointVisible(&viewOrigin, &pi->head);
						pi->head.z += 8.f;
						CalcPlayerRect(ent, pi);
						pi->head.z -= 8.f;
						CalculateEntityVelocity(ent, pi, &pi->velocity);
						if (ent->IsHostile()) {
